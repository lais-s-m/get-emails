...
async function getAllEmailsFromAllTables(url: string): Promise<string[]> {
...
// CLI usage: npx ts-node get-emails-puppeteer.ts <url>
if (require.main === module) {
  const url = process.argv[2];
  if (!url) {
    console.error('Usage: npx ts-node get-emails-puppeteer.ts <url>');
    process.exit(1);
  }
